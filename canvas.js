...
function createPhysicsSimApp(mountPoint) {
    ...
    function update(dt) {
        ...
        for (const ball1 of balls) {
            for (const ball2 of balls) {
                ...
                if (
                    ...
                    sqrMag(ball1.posX - ball2.posX, ball1.posY - ball2.posY) >
                    (ball1.size + ball2.size) * (ball1.size + ball2.size)
        ...
        for (const ball of balls) {
            if (
                (ball.posX + ball.size > screen.width && ball.velX > 0) ||
                (ball.posX - ball.size < 0 && ball.velX < 0)
            ) {
                ball.velX = -ball.velX;
            }
            if (
                (ball.posY + ball.size > screen.height && ball.velY > 0) ||
...
function reflect(v1, v2, n1, n2) {
    ...
    return [v1 - 2 * vInN * n1, v2 - 2 * vInN * n2];
}
